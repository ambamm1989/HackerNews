...
async function submitNewStory(evt){
  ...
  function putUserStoriesOnPage(){
    ...
    if(currentUser.$ownStories.length === 0){
      $ownStories.append("<h5>Stories not add by user yet!</h5>");
  ...
  function putFavoritestOnPage(){
    console.debug(("putFavoritestListOnPage");
    $favitedStories.empty();

    if(currentUser.favorites.length === 0){
      $favitedStories.append("<h5> No Favorites Added!</h5>");
    } else {
      for(let story of currentUser.favorites){
        const $story = generateStoryMarkup(story);
        $favitedStories.append($story);
      }
    }
    $favitedStories.show();
  }
